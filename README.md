...
| **[`homelab-k8s-talos`](https://github.com/pedrogriff/homelab-k8s-talos)** | Declarative, immutable multi-node distributed Kubernetes platform on **Proxmox VE** and **Talos Linux (v1.13)**. Serves containerized **FastMCP Compensation Microservices** with anti-affinity across Control Plane and Worker nodes, **Authentik Enterprise OIDC Single Sign-On**, **SOPS & Age** asymmetric GitOps secrets, **ArgoCD** continuous delivery, **Cilium eBPF** kernel routing & **Hubble UI** network radar, **Prometheus & Grafana** observability, and **cert-manager** PKI. | Multi-Node Kubernetes v1.36, Talos Linux, Authentik OIDC, FastMCP, SOPS/Age, Cilium (eBPF), ArgoCD, Prometheus, Grafana, cert-manager, Proxmox VE, IaC | Active |
