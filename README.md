...
| **[`homelab-k8s-talos`](https://github.com/pedrogriff/homelab-k8s-talos)** | Declarative, immutable multi-node distributed Kubernetes platform on **Proxmox VE** and **Talos Linux (v1.13)**. Features self-hosted autoscaling **Actions Runner Controller (ARC)** CI/CD runners, containerized **FastMCP Compensation Microservices** with anti-affinity, **Authentik Enterprise OIDC Single Sign-On**, **SOPS & Age** asymmetric GitOps secrets, **ArgoCD** continuous delivery, **Cilium eBPF** kernel routing & **Hubble UI** network radar, **Prometheus & Grafana** observability, and **cert-manager** PKI. | Multi-Node Kubernetes v1.36, Talos Linux, Actions Runner Controller (ARC), Authentik OIDC, FastMCP, SOPS/Age, Cilium (eBPF), ArgoCD, Prometheus, Grafana, cert-manager, Proxmox VE, IaC | Active |
